...
function overlayCalendar() {
    return {
        ...
        readConfig() {
            ...
            const minDate = element.dataset.minDate || null;
            const maxDate = element.dataset.maxDate || null;
            const disabledDates = element.dataset.disabledDates ? JSON.parse(element.dataset.disabledDates) : [];
            return {
        ...
        init() {
            ...
            if (this.externalTriggerSelector && !this._externalListenersBound) {
                try {
                    ...
                    console.warn('OverlayCalendar: Failed to bind external trigger', e);
                }
            }
        },
        ...
        isDateValid(date) {
            ...
            // Проверяем отключенные даты
            if (this.config.disabledDates.includes(dateStr)) {
                return false;
            }
