...
class OverlayCalendar implements Renderable
{
    ...
    protected ?string $minDate = null; // минимальная дата
    protected ?string $maxDate = null; // максимальная дата
    protected array $disabledDates = []; // отключенные даты
    ...
    public function render(): string
    {
        ...
        $dataAttributes = [
            ...
            'data-disabled-dates' => json_encode($this->disabledDates),
        ];
        ...
        <div class="relative" x-data="overlayCalendar()" style="' . $inlineStyle . '" ' . $dataAttrString . '>
            ...
            <template x-if="showInternalTrigger">
                <div class="calendar-trigger" 
                     ...
                     role="button"
                     tabindex="0"
                     :aria-label="config.inputPlaceholder || config.placeholder"
                     :aria-expanded="isOpen"
                     :aria-haspopup="true">
                    <div class="calendar-display">
                        <span x-text="displayText" class="calendar-text"></span>
                        <div x-html="config.inputIcon" class="calendar-icon" :style="{ color: config.inputIconColor }" aria-hidden="true"></div>
                    </div>
                </div>
            </template>
    ...
    private function includeAssets(): void
    {
        ...
        if (!$assetsIncluded) {
            // Подключаем CSS и JS с версионированием для кеширования
            $version = filemtime(public_path('css/overlay-calendar.css')) ?: time();
            echo '<link rel="stylesheet" href="/css/overlay-calendar.css?v=' . $version . '">';
            echo '<script src="/js/overlay-calendar.js?v=' . $version . '"></script>';
    ...
    private function renderIcon(string $icon): string
    {
        ...
        try {
            ...
            return $rendered ?: $this->getDefaultIcon();
        } catch (\Exception $e) {
    ...
    {
        return '<svg class="calendar-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>';
    }
}
